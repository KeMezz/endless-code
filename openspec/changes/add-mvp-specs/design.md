...
| 항목 | 한계 | 비고 |
|------|------|------|
| 세션당 메시지 버퍼 | 50MB | 최근 1,000개 메시지 |
| Tree-sitter 파서 캐시 | 100MB | 언어당 최대 10MB |
| 파일 뷰어 캐시 | 20MB | LRU 정책 |
| WebSocket 버퍼 (연결당) | 10MB | 재연결 대기용 |
| 전체 앱 메모리 | 500MB | 경고 임계치 |

### Concurrency Limits

| 항목 | 한계 | 초과 시 동작 |
|------|------|-------------|
| 동시 CLI 세션 | 5개 | 유휴 세션 종료 안내 |
| 동시 WebSocket 연결 | 100개 | 503 Service Unavailable |
| 파일 동시 로드 | 3개 | 큐잉 (FIFO) |
| 백그라운드 작업 | 4개 | 큐잉 |
...
|------|------|-------------|
| 단일 메시지 | 1MB | 청크 분할 전송 |
| 코드 블록 | 1MB | "접기" 상태로 표시, 경고 |
| 파일 미리보기 | 1MB | 부분 로드 + 경고 |
| Diff 파일 수 | 100개 | 페이지네이션 |
| 디렉토리 깊이 | 50레벨 | 경고 표시, 하위 무시 |
| 디렉토리 항목 수 | 1,000개 | 페이지네이션 |

### Response Time Targets

| 작업 | 목표 | 측정 기준 |
|------|------|----------|
| 메시지 수신→UI 표시 | 100ms | P95 |
| 파일 트리 초기 로드 | 500ms | P95 |
| 코드 하이라이팅 | 50ms | 1,000줄 기준 |
| 검색 결과 표시 | 200ms | P95 |
| WebSocket 재연결 | 5초 | 첫 번째 시도 |
