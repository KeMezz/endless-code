...
시스템은 세션 및 프로젝트 관련 오류를 처리해야 합니다(SHALL).

#### Scenario: 프로젝트 디렉토리 삭제됨
- **GIVEN** 프로젝트 목록에 항목 존재
- **WHEN** 해당 프로젝트의 디렉토리가 삭제됨
- **THEN** 다음 새로고침 시 목록에서 제거
- **AND** 활성 세션이 있었다면 "프로젝트 삭제됨" 알림

#### Scenario: 세션 파일 손상
- **GIVEN** 세션 히스토리 로드 요청
- **WHEN** JSONL 파일이 손상됨 (부분 쓰기, 인코딩 오류)
- **THEN** 유효한 라인까지만 로드
- **AND** 손상된 라인 수 경고 표시
- **AND** 손상된 파일 백업 생성 (.bak)

#### Scenario: 읽기 권한 없음
- **GIVEN** 프로젝트 디렉토리 스캔
- **WHEN** 읽기 권한 없는 디렉토리 발견
- **THEN** 해당 디렉토리 건너뛰기
- **AND** 경고 로그 기록
- **AND** 접근 가능한 프로젝트만 표시
...
#### Scenario: 세션 상태 불일치
- **GIVEN** 세션 재개 요청
- **WHEN** 세션 파일은 있지만 CLI 프로세스 상태와 불일치
- **THEN** 세션 상태 자동 복구 시도
- **AND** 복구 실패 시 "새 세션 시작" 옵션 제공

#### Scenario: 동시 세션 충돌
- **GIVEN** 동일 프로젝트에 세션 존재
- **WHEN** 다른 클라이언트가 같은 세션 접근 시도
- **THEN** 읽기 전용 모드로 접근 허용
- **OR** "세션 점유 중" 알림 표시
- **AND** 현재 사용자에게 알림

#### Scenario: 디스크 공간 부족
- **GIVEN** 세션 진행 중 메시지 저장
- **WHEN** 디스크 공간 부족
- **THEN** 저장 실패 경고 표시
- **AND** 메모리에 최근 100개 메시지 보관
- **AND** 공간 확보 후 재시도 옵션 제공
