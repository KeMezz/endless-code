# Tasks: MVP Implementation

## 1. Server Core

### 1.1 프로젝트 기반 설정
- [ ] 1.1.1 Vapor 패키지 추가 및 서버 엔트리포인트
- [ ] 1.1.2 공통 모델 정의 - Message, Session, Project 타입
- [ ] 1.1.3 설정 모델 및 환경 변수 처리

### 1.2 CLI 프로세스 관리
- [ ] 1.2.1 ProcessRunner - 기본 Process 래퍼, 시작/종료
- [ ] 1.2.2 ProcessRunner - stdin 쓰기, stdout/stderr 스트리밍
- [ ] 1.2.3 ClaudeCodeManager - ProcessRunner 통합, 세션별 프로세스 관리
- [ ] 1.2.4 ClaudeCodeManager - 재시작, 타임아웃, 에러 복구

### 1.3 JSONL 파싱
- [ ] 1.3.1 JSONLParser - 라인 버퍼링 및 기본 파싱
- [ ] 1.3.2 메시지 타입 분류 - message, tool_use, tool_result
- [ ] 1.3.3 방어적 파싱 - 누락 필드, 알 수 없는 타입 처리

### 1.4 세션 관리
- [ ] 1.4.1 ProjectDiscovery - ~/.claude/projects 스캔
- [ ] 1.4.2 SessionStore - 세션 CRUD, 메모리 저장소
- [ ] 1.4.3 SessionHistoryLoader - JSONL 히스토리 파일 로딩

### 1.5 WebSocket API
- [ ] 1.5.1 WebSocket 라우트 및 연결 관리
- [ ] 1.5.2 메시지 수신 → CLI stdin 전달
- [ ] 1.5.3 CLI stdout → 클라이언트 브로드캐스트
- [ ] 1.5.4 연결 상태 동기화 및 재연결 지원

### 1.6 대화형 프롬프트
- [ ] 1.6.1 AskUserQuestion 감지 로직
- [ ] 1.6.2 프롬프트 상태 관리 및 응답 주입

### 1.7 서버 테스트
- [ ] 1.7.1 ProcessRunner 단위 테스트
- [ ] 1.7.2 JSONLParser 단위 테스트
- [ ] 1.7.3 WebSocket 통합 테스트

## 2. macOS App - 공통 컴포넌트

### 2.1 앱 구조 설정
- [ ] 2.1.1 앱 구조 - NavigationSplitView, 탭 구조
- [ ] 2.1.2 서버 연결 서비스 - WebSocket 클라이언트
- [ ] 2.1.3 앱 상태 관리 - @Observable 모델

### 2.2 프로젝트 브라우저
- [ ] 2.2.1 프로젝트 목록 View
- [ ] 2.2.2 프로젝트 카드 컴포넌트
- [ ] 2.2.3 검색 및 필터링

## 3. macOS App - 채팅

### 3.1 메시지 표시
- [ ] 3.1.1 MessageBubble - 사용자/어시스턴트 구분
- [ ] 3.1.2 MessageList - 스크롤, 날짜 구분선
- [ ] 3.1.3 스트리밍 메시지 표시 - 타이핑 인디케이터

### 3.2 코드 블록
- [ ] 3.2.1 CodeBlockView - 기본 레이아웃, 언어 표시
- [ ] 3.2.2 Tree-sitter 하이라이팅 통합
- [ ] 3.2.3 복사 버튼 및 피드백

### 3.3 입력 및 전송
- [ ] 3.3.1 MessageInputView - 텍스트 입력, 여러 줄
- [ ] 3.3.2 전송 로직 및 상태 표시

### 3.4 도구 사용 표시
- [ ] 3.4.1 ToolUseView - 도구 이름, 파라미터
- [ ] 3.4.2 ToolResultView - 결과, 접기/펼치기

## 4. macOS App - 파일 탐색기

### 4.1 디렉토리 트리
- [ ] 4.1.1 FileTreeView - 재귀 트리 구조
- [ ] 4.1.2 FileTreeNode - 확장/축소, 아이콘
- [ ] 4.1.3 파일 타입별 아이콘 매핑

### 4.2 파일 뷰어
- [ ] 4.2.1 FileContentView - 기본 텍스트 표시, 라인 번호
- [ ] 4.2.2 신택스 하이라이팅 적용
- [ ] 4.2.3 바이너리/대용량 파일 처리

### 4.3 Git 상태
- [ ] 4.3.1 Git 상태 조회 서비스
- [ ] 4.3.2 파일 트리에 상태 표시

## 5. macOS App - Diff 뷰어

### 5.1 Diff 파싱
- [ ] 5.1.1 UnifiedDiff 파서 - Hunk 추출
- [ ] 5.1.2 DiffLine 모델 - 추가/삭제/컨텍스트

### 5.2 Diff 표시
- [ ] 5.2.1 DiffHunkView - 라인별 렌더링
- [ ] 5.2.2 이중 라인 번호 표시
- [ ] 5.2.3 Diff 내 신택스 하이라이팅

### 5.3 파일 목록
- [ ] 5.3.1 DiffFileList - 파일별 그룹화
- [ ] 5.3.2 통계 표시 - 추가/삭제 라인 수

## 6. macOS App - 기타 UI

### 6.1 마크다운 뷰어
- [ ] 6.1.1 MarkdownUI 통합 및 스타일링

### 6.2 대화형 프롬프트 UI
- [ ] 6.2.1 PromptDialog - 질문 표시
- [ ] 6.2.2 옵션 버튼 및 멀티 셀렉트
- [ ] 6.2.3 직접 입력 필드

### 6.3 설정 화면
- [ ] 6.3.1 SettingsView - 서버, CLI 경로 설정
- [ ] 6.3.2 Keychain 토큰 관리

### 6.4 메뉴바
- [ ] 6.4.1 MenuBarExtra - 서버 상태 표시
- [ ] 6.4.2 빠른 액션 메뉴

## 7. iOS App

### 7.1 서버 연결
- [ ] 7.1.1 ServerSettingsView - 주소/토큰 입력
- [ ] 7.1.2 연결 테스트 및 상태 표시
- [ ] 7.1.3 Bonjour 서버 발견

### 7.2 모바일 UI 적응
- [ ] 7.2.1 탭 기반 네비게이션 구조
- [ ] 7.2.2 채팅 뷰 모바일 최적화 - 키보드 대응
- [ ] 7.2.3 코드 블록 - 가로 스크롤, 핀치 줌
- [ ] 7.2.4 파일/Diff 뷰어 모바일 레이아웃

### 7.3 네트워크 처리
- [ ] 7.3.1 연결 상태 모니터링 및 재연결
- [ ] 7.3.2 백그라운드/포그라운드 전환 처리

## 8. 테스트 및 문서화

### 8.1 macOS 테스트
- [ ] 8.1.1 ViewModel 단위 테스트
- [ ] 8.1.2 View 스냅샷 테스트

### 8.2 iOS 테스트
- [ ] 8.2.1 iOS 통합 테스트

### 8.3 문서화
- [ ] 8.3.1 README - 설치 및 사용법
- [ ] 8.3.2 개발자 가이드

## Dependencies

```
1.1 → 1.2 → 1.3 → 1.4 → 1.5 → 1.6 (순차)
      ↓
2.1 → 2.2 (macOS 공통)
      ↓
┌─────┼─────┬─────┐
3.x   4.x   5.x   6.x (병렬 가능)
      ↓
     7.x (iOS, macOS 완료 후)
      ↓
     8.x (테스트/문서)
```
